...
#                num_threads: int = 4,
#                multi: bool = False,
#                limit_cpu_clock: bool = True,
#                cpu_max_freq: str = "4.0GHz"):
    
#     """
#     Generate a shell script to run SOP multiple times.
    
#     If multi=True, use GNU parallel (with progress bar); otherwise run sequentially
#     with a bash progress bar.

#     New options:
#       - limit_cpu_clock: if True, try to cap CPU max frequency before running
#       - cpu_max_freq: max CPU frequency, e.g. "4.0GHz", "3800MHz"
#     """
#     if dim not in (2, 3):
#         return "please, enter with dim = 2 or 3"
...
#     cpu_limit_block = f"""\
# # --- optional CPU clock limiter ---
# LIMIT_CPU_CLOCK={"1" if limit_cpu_clock else "0"}
...
#       if [[ $EUID -eq 0 ]]; then
#         cpupower frequency-set -g "$OLD_GOVERNOR" >/dev/null 2>&1 || true
...
#   if ! command -v cpupower >/dev/null 2>&1; then
#     echo "[WARN] cpupower not found. Skipping CPU frequency limit."
#     return 0
#   fi

#   # save current governor, if available
#   if [[ -f /sys/devices/system/cpu/cpu0/cpufreq/scaling_governor ]]; then
#     OLD_GOVERNOR=$(cat /sys/devices/system/cpu/cpu0/cpufreq/scaling_governor 2>/dev/null || true)
#   fi

#   echo "[INFO] Trying to cap CPU max frequency to $CPU_MAX_FREQ"

#   if [[ $EUID -eq 0 ]]; then
#     cpupower frequency-set -g ondemand >/dev/null 2>&1 || true
#     cpupower frequency-set -u "$CPU_MAX_FREQ" >/dev/null 2>&1 || {{
#       echo "[WARN] Could not set CPU max frequency."
#       return 0
#     }}
#   elif command -v sudo >/dev/null 2>&1 && sudo -n true >/dev/null 2>&1; then
#     sudo -n cpupower frequency-set -g ondemand >/dev/null 2>&1 || true
#     sudo -n cpupower frequency-set -u "$CPU_MAX_FREQ" >/dev/null 2>&1 || {{
#       echo "[WARN] Could not set CPU max frequency with sudo."
#       return 0
#     }}
#   else
#     echo "[WARN] No privilege to change CPU frequency. Run with sudo/root or disable limit_cpu_clock."
#     return 0
#   fi

#   echo "[INFO] CPU max frequency cap applied: $CPU_MAX_FREQ"
# }}
