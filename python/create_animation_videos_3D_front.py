...
for type_base in types_list:
    frames_dir = os.path.join(
        PROJECT_ROOT, "animate", f"L_{L}_nc{nc}", f"type_base_{type_base}_front"
    )
    ...
    if os.path.isdir(cropped_dir):
        print(f"\n[animation_3D_front] Usando frames recortados de {cropped_dir}")
        input_dir = cropped_dir
        output_dir = os.path.join(frames_dir, "videos")
    else:
        print(f"\n[animation_3D_front] Usando frames originais de {frames_dir}")
        input_dir = frames_dir
        output_dir = frames_dir

    os.makedirs(output_dir, exist_ok=True)

    print(f"[animation_3D_front] Montando animações da frente para: {type_base}")
    ...
    print(f"✓ Animações da frente criadas para {type_base}")

print("\n✓ Todas as animações da frente foram criadas com sucesso!")
